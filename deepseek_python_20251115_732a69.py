...
def run_flask():
    """Run Flask web server for status dashboard"""
    try:
        logger.info(f"Starting Flask web server on port {config.WEBHOOK_PORT}...")
...
def main():
    ...
    try:
        ...
        if config.USE_WEBHOOK:
            ...
            # Start Flask web server in a separate thread for status dashboard
            flask_thread = threading.Thread(target=run_flask, daemon=True)
            flask_thread.start()
            print("🌐 Flask status dashboard started")
